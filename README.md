...
Четыре вещи здесь неочевидны и были найдены билдом, а не чтением доков:
...
* **`PROOT_TMP_DIR` обязателен.** `libproot.so` — сборка Termux, и путь своего
  временного каталога (`/data/data/com.termux/files/usr/tmp/`) она хранит
  зашитым. Чужому приложению туда не написать, и proot падает с
  `can't create temporary directory` → `can't create glue rootfs` →
  `execve("/usr/bin/env"): No such file or directory`. Последний вывод выглядит
  как сломанный rootfs, но на деле просто не создались glue-бинари. Лечится
  `RuntimeSpec.prootEnv()`, которая указывает `PROOT_TMP_DIR` в `runtime/tmp`;
  задать её внутри гостевого `env -i` нельзя — proot читает своё окружение до
  exec.
...
Маркер распаковки хранит **размер ассета**, а не только факт «готово»: payload
пересобирается между билдами, `summary()` при этом не меняется, и слепой маркер
навечно оставил бы установленное устройство на старых файлах.
